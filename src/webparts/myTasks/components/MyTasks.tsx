...
// import { IMyRequestProps } from "./IMyTasksProps";
...
import { getAllDMSTasks, getDataByID, getMyRequest, getRequestListsData, gteDMSApproval } from "../../../APISearvice/ApprovalService";
...
const MyTasksContext = ({ props }: any) => {
  ...
  const ApiCall = async (status: String) => {
    ...
    // console.log(myrequestdatadms, "myrequestdatadms");
    setMyApprovalsData(await getAllDMSTasks(sp, status));
  ...
  const Breadcrumb = [
...

    {
      ...
      ChildComponent: "My Tasks",

      ChildComponentURl: `${siteUrl}/SitePages/MyTasks.aspx`,
  ...
  const handleStatusChange = async (name: string, value: string) => {
    if (value === "") {
      ...
      let mydmsData = await getAllDMSTasks(sp, value);
      ...
      if (activeTab == "Intranet") {
        ...
        setMyApprovalsData(await getAllDMSTasks(sp, value));
        console.log(myApprovalsData, "myApprovalsData");
